...
async fn test_true_codex_ask_pend_roundtrip() {
    ...
    assert!(
        ...
        "pend returned before the real provider reply arrived"
    );
    let result = wait_task.await.unwrap().unwrap();
    assert_eq!(result["status"], "COMPLETED");
    assert!(result["reply_text"].as_str().unwrap().contains('1'));
