...
fn migrate_jobs_governance_binding(conn: &Connection) -> Result<(), AhError> {
...
fn migrate_agents_lifecycle_id(conn: &Connection) -> Result<(), AhError> {
    add_column_if_missing(
        conn,
        "agents",
        "lifecycle_id",
    ...
    .map_err(|err| {
        AhError::DbConstraintViolation(format!("backfill agents.lifecycle_id: {err}"))
    })?;
    Ok(())
}
...
fn migrate_sessions_master_provider(conn: &Connection) -> Result<(), AhError> {
    add_column_if_missing(
        conn,
        "sessions",
        "master_provider",
