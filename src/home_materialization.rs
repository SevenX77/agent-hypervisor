...
fn prepare_claude_overrides(
    ...
    if let Some(ctx) = active_hook_push_ctx(hook_push_ctx, "claude") {
        hook_specs.extend(
            ah_owned_hook_events("claude")
                .iter()
                .map(|event| materialized_ah_hook(ctx, event)),
        );
    }
...
fn merge_antigravity_hooks(
    ...
    for hook in hooks {
        ...
        if antigravity_event_uses_direct_handlers(&hook.event) {
            event_hooks.push(Value::Object(hook_item));
        } else {
...
fn inject_antigravity_hook_push(root: &mut Map<String, Value>, ctx: &HookPushContext) {
    ...
    for event in ah_owned_hook_events("antigravity") {
        let event_hooks = named_hook
        ...
        event_hooks.retain(|item| !is_ah_owned_hook_item(item));
        let hook = materialized_ah_hook(ctx, event);
        event_hooks.push(serde_json::json!({
            "type": hook.item.hook_type,
            "command": hook.item.command,
            "timeout": hook.item.timeout,
        }));
    }
}

fn antigravity_event_uses_direct_handlers(event: &str) -> bool {
    matches!(event, "PreInvocation" | "PostInvocation" | "Stop")
}
...
fn builtin_rules_target(provider: &str, home_root: &Path) -> Option<PathBuf> {
    crate::provider::adapter(provider).and_then(|adapter| adapter.rules_target(home_root))
}
...
pub fn build_ah_hook_command(ctx: &HookPushContext, event: &str) -> HookItem {
    ...
    HookItem {
        ...
        command: format!(
            "CCB_SOCKET={socket} {ah_bin} agent notify --agent-id {} --event {event} --provider {} --lifecycle-id {} --socket {socket} --hook-json{hook_debug_log}",
            ctx.agent_id, ctx.provider, ctx.lifecycle_id
        ),
...
pub(crate) fn resolve_ah_binary_strict(current_exe: &Path) -> Option<PathBuf> {
    if let Some(dir) = current_exe.parent() {
        ...
        // the candidate `ah` binary is one directory above. Resolving that
        // sibling-of-deps prevents a hook smoke test from silently invoking an
        // older globally installed CLI.
        if dir.file_name().and_then(|name| name.to_str()) == Some("deps")
            && let Some(profile_dir) = dir.parent()
        {
            let candidate = profile_dir.join("ah");
            if candidate.is_file() {
                return Some(candidate);
            }
        }
...
fn hook_timeout_for_provider(provider: &str) -> u64 {
    crate::provider::adapter(provider)
        .map(|adapter| adapter.hook_timeout_secs())
        .unwrap_or(5)
}
...
fn ah_owned_hook_events(provider: &str) -> Vec<&'static str> {
    crate::provider::adapter(provider)
        .map(|adapter| {
            adapter
                .observation_spec()
                .materialized_hook_events()
                .collect()
        })
        .unwrap_or_default()
}
...
fn merge_codex_hook_push(codex_home: &Path, ctx: &HookPushContext) -> Result<(), CcbdError> {
    ...
    let mut root = read_codex_hooks_for_hook_push(&hooks_path);
    let hooks_root = object_entry(&mut root, "hooks");
    for event in ah_owned_hook_events("codex") {
        let event_hooks = hooks_root
            .entry(event.to_string())
            .or_insert_with(|| Value::Array(vec![]));
        if !event_hooks.is_array() {
            *event_hooks = Value::Array(vec![]);
...
share /mnt/linux-share 9p rw,relatime,trans=virtio,version=9p2000.L 0 0\n";
    ...
    fn test_antigravity_hook_push_enables_json_hooks_gate_and_preserves_config() {
        ...
        let ctx = HookPushContext {
            ...
            lifecycle_id: "lifecycle-a1".to_string(),
    ...
    fn claude_master_hook_push_installs_ups_and_stop_with_master_sentinel() {
        ...
        let ctx = HookPushContext {
            ...
            lifecycle_id: "master-7".to_string(),
            ahd_socket_path: PathBuf::from("/tmp/ahd.sock"),
            enabled: true,
        };
    ...
    #[test]
    fn claude_worker_hook_push_installs_userpromptsubmit_and_stop() {
        use super::{HomeLayoutRole, HookPushContext, prepare_claude_overrides, read_json_object};
        use std::path::PathBuf;
        use tempfile::TempDir;
        ...
        let ctx = HookPushContext {
            ...
            provider: "claude".to_string(),
            lifecycle_id: "worker-lifecycle-1".to_string(),
            ahd_socket_path: PathBuf::from("/tmp/ahd.sock"),
            enabled: true,
        };
        ...
        assert!(
            hooks["UserPromptSubmit"]
                .to_string()
                .contains("--event userpromptsubmit")
        );
        assert!(hooks["Stop"].to_string().contains("--event stop"));
    }
    ...
    #[test]
    fn resolve_ah_binary_strict_finds_cargo_profile_binary_from_deps() {
        use super::resolve_ah_binary_strict;

        let temp = tempfile::TempDir::new().unwrap();
        let deps = temp.path().join("debug/deps");
        std::fs::create_dir_all(&deps).unwrap();
        let ah = temp.path().join("debug/ah");
        std::fs::write(&ah, b"candidate").unwrap();

        assert_eq!(
            resolve_ah_binary_strict(&deps.join("integration-test")),
            Some(ah)
        );
    }
